---
name: prd-compress
description: Run the once-per-tier PRD compression sweep (Flow 2) — draft a tier's fat prose down to intent + acceptance criteria + pointers, archive the demoted text verbatim, file a reel-back plan + R-item, and land only on human approval. DORMANT until that tier's taste review (R-item) closes. Fable-routed judgment work. User-invoked only.
argument-hint: "Which tier to compress (e.g. T0) — only after its taste R-item has closed"
disable-model-invocation: true
---

# PRD compress (Flow 2 — the once-per-tier compression event)

> Encodes **Flow 2** of the D-117 "PRD on a diet" decision
...
1. **Gated on the tier's taste review closing.** The trigger is the tier's
   **human taste review (R-item) closing** (Q3) — *not* code-ship. If the tier's
   R-item in
   [`review.md`](../../../project/human-in-the-loop/review.md) is still open
   (e.g. **R1** for the T0 M0–M4 demo), **stop** — the tier isn't ready to
...
2. **Routing: this is FABLE work.** Compressing signed canon is judgment-dense
   ...
   (silent meaning-loss) and **no gate catches them** (R2/Flow 4). An Opus
   session should route the sweep itself to **Fable** unless the human steers
   otherwise (D-124). *Authoring* this skill was transcription; *running* it is
   the sweep.

## The sweep (Flow 2, verbatim)

1. **Trigger confirmed** — the tier's taste review (R-item) has closed (guard 1).
2. **Draft the tier sweep** (Q6 granularity — surgical **subsection** edits, not
   ...
   The build is now the truth of detail (R2); the PRD stops re-stating it.
...
4. **Insert gen-regions** where **build == end-state intent** (Q2's C half),
   ...
   pattern — markers + `--check`). Numbers stay OUT (§4 ripple-frozen, D-021).
5. **File for review** — the sweep is a
   [`docs/plans/`](../../../docs/plans) **reel-back plan + an R-item** in
...
## Content-preserving-transform discipline (A15)
...
real, R-item-gated, human-approved sweep.
