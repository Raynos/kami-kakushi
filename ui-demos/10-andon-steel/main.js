// ui-demos/10-andon-steel — ANDON STEEL. Andon's two-light composition (left
// document-box rail · centre action-desk · right event-log WINDOW · crest-to-
// seal cursor) reskinned in Damascene's blackened-steel BIMETAL: silver = state/
// chrome/labels/top-rim, gold = value/numerals/progress/keyline/face, vermillion
// = commit/danger (the lone hot accent). The warm/cool temperature-as-mode grade
// is GONE — one uniform steel skin, no nunome cross-hatch, no grain, no lamp-pool.
// The log window is a recessed steel WELL (silver-on-dark, not cream paper). The
// cold open is 06 Washi's behaviour: a staged reveal ending in a char-by-char
// TYPEWRITTEN lede (the "GBA text scroll"). ALL of Andon's machinery is intact
// (subscribe→HTML render, reconcile, the crest-to-seal commit bloom, the whole
// mobile recomposition + every iOS trap). All data from ../shared/*.js.
...
const ui = {
  ...
  coldTimers: [],     // cold-open staged-reveal timers (cleared on replay/leave)
  coldPlayed: false,  // guards the one-shot staged reveal (never re-runs on tick)
};
...
// The cold open — 06 Washi's staged behaviour, in steel/silver/gold. The lede is
// an EMPTY <p> that carries its text in data-lede; playColdOpen types it in
// char-by-char (the "GBA text scroll"). Title/sub/lede/verb are .co-item's,
// each opacity:0 until JS adds .in. The crest is a static header mark.
function coldHTML() {
  return `<div class="cold" data-co="1">
    <div class="cold-inner">
      <div class="cold-crest">◆ ◆ ◆</div>
      <h1 class="cold-title co-item" data-co-step="0">${esc(D.COLD_OPEN.title)}</h1>
      <div class="cold-sub co-item" data-co-step="1"><span class="rule"></span>${esc(D.COLD_OPEN.subtitle)}<span class="rule"></span></div>
      <p class="cold-lede co-item" data-co-step="2" data-lede="${esc(D.COLD_OPEN.lede)}"></p>
      <button class="seal seal-lg cold-verb co-item" data-co-step="3" data-act="open-eyes">${esc(D.COLD_OPEN.verb)}</button>
    </div>
  </div>`;
}

// staged reveal: title → subtitle → typewritten lede → the lone verb. Timers are
// tracked in ui.coldTimers so a replay can clear them; the coldPlayed flag makes
// it fire exactly ONCE (never re-runs on the 480ms passive tick). RM reveals all
// at once and sets the lede text with no typing.
function playColdOpen(ov) {
  ...
  const lede = ov.querySelector('.cold-lede');
  ...
  at(1700, () => {
    ...
    lede?.classList.add('co-typing');
    let i = 0;
    const step = () => {
      if (!lede.isConnected) return;
      lede.textContent = ledeText.slice(0, ++i);
      if (i < ledeText.length) {
        ui.coldTimers.push(setTimeout(step, 24));
      } else {
        lede.classList.remove('co-typing');
...
function renderOverlay(s) {
  ...
  // leaving the cold open — reset the one-shot guard + drop any pending timers,
  // so a later re-entry (e.g. "Play intro") replays the staged reveal cleanly.
...
// headless QA / review handle (mirrors 07's engine exposure)
window.__andonsteel = { eng, sel, ui };
