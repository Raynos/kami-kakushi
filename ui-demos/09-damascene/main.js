// ui-demos/09-damascene/main.js — Damascene Duet: a fork of 04-lacquer, re-skinned
// into blackened-steel nunome-zogan. Same proven machinery (region-diffing
// reconcile, append-only log feed, rAF meter fills, the whole 04 mobile
// recomposition) — the re-imagination is MATERIAL and SEMANTIC: silver = state,
// gold = value, vermillion = commit. The signature is THE STRIKE (below). All
// data/logic from ../shared/*.js — nothing invented, urushi + paper dropped.
...
function headerHTML(s) {
  ...
  const r = s.resources;
  const left = `<div class="house"><span class="crest">K</span><span class="house-name">${esc(D.HOUSE_MARK)}</span></div>`;
...
function navHTML(s) {
  return sel.visibleTabs(s).map((t) => {
    ...
    return `<button class="tab${t.id === ui.tab ? ' on' : ''}${fresh ? ' glint' : ''}" data-act="tab" data-tab="${t.id}">${esc(t.label)}</button>`;
  }).join('<span class="tab-sep"></span>');
}
...
// ── THE STRIKE ────────────────────────────────────────────────────────────────
// The signature: every damascene lozenge (.seal — the CTA + the summons + the
// +1 stamps) SEATS under the thumb while vermillion floods up through its gold
// inlay from center, then bleeds slowly back. A pointerdown class-retrigger
// drives the flood so it survives an innerHTML reconcile and outlives the press
// (the CSS keyframe carries the ~280ms bleed-back). Reduced-motion collapses it.
document.addEventListener('pointerdown', (e) => {
  const el = e.target.closest('.seal:not(:disabled)');
  if (!el) return;
  el.classList.remove('struck');
  void el.offsetWidth; // reflow — restart the keyframe every strike
  el.classList.add('struck');
});

eng.subscribe(() => renderAll());

// DEV play API — drive/observe the fork headlessly (mirrors 04's engine export).
window.__damascene = { eng, sel };
