// ui-demos/08-night-seal/main.js
// Night Seal — a cold indigo-lacquer estate through which travels one warm
// object. Fork of 01-moonlit's render/reconcile machinery; the only behavioural
// addition is the ink-bloom (a vermillion seal-impression stamped where you
// commit). Render + reconcile over the shared mock engine.
...
// ── the ink-bloom ────────────────────────────────────────────────────────────
// A transient vermillion seal-impression fired ONLY on a discrete player commit.
// It lives in a persistent layer OUTSIDE #app, so the dispatch→render (which
// wipes app.innerHTML) never kills it mid-animation. It NEVER fires on the
// passive 480ms tick or on auto-toggle repeats — only a real click on one of
// the committing verbs below. This is where warmth "blooms where you commit".
const bloomLayer = document.createElement('div');
bloomLayer.className = 'bloom-layer';
bloomLayer.setAttribute('aria-hidden', 'true');
document.body.appendChild(bloomLayer);

const COMMIT_ACTS = new Set([
  'rake_rice', 'do_activity', 'buy_item', 'sell_rice', 'improve_estate',
  'cook_meal', 'eat_rice', 'fight', 'face_wolf', 'move_to', 'deposit',
  'withdraw', 'spend_attribute', 'accept_quest', 'begin_rung_beat', 'vn_choose',
]);

function fireBloom(el) {
  ...
  // scale the impression to the sealed target — it reads as sealing THIS row
  const size = Math.min(Math.max(r.height * 1.7, 42), 96);
...
app.addEventListener('click', (e) => {
  ...
  // engine intents — stamp the ink-bloom on the sealed target for real player
  // commits only (never auto-toggles, never the passive tick)
  if (COMMIT_ACTS.has(act)) fireBloom(el);
