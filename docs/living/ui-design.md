...
> **SNAPSHOT-CLASS — replace in place, never append (D-126). Hard cap: 400 lines.** The `src/ui` renderer is
...
> **PROVISIONAL — pending R9 (D-126).** The human rates the woodblock/ink direction *softer canon than this
...
declining house*, never a flashy mobile idler (anchored to 1780, D-105). The **event log is the heart of the
...
- **attr-*** — the five-attribute pigments (F66): STR 弁柄 iron-red · AGI 若竹 green · INT 紫 murasaki ·
...
Q52/D-013: a font, not an art asset; Google dynamic subsetting rejected for offline purity).
...
### 4.2 The app shell (F1–F8; taste.md 19–20)
...
**The multi-panel workspace (D-106, LOCKED by F69/F70): 屏風 folding columns + soft cards** (V6B×V7C). The
...
raw `vw` — it balloons on wide screens, F117); the Work fold holds the majority and fills as panels unlock.
...
to content and never clip (F94); padding is consistent across ALL tabs (F98). DEV chrome is a fixed overlay
...
### 4.4 The seven-tab IA (D-112 → D-119)
...
qualify. Reveal is gated by real content (D-055), one-per-beat (the D-119 stagger). One capability, one tab
(taste.md 1):
...
| Inventory | 蔵 | The storehouse/bank (§5.12) + personal belongings (D-111). |
| Character | 己 | Attributes, skills, bestiary (sections, not screens). |
| Combat | 武 | The fight loop + stance + the node watch (D-100). |
| Quests | 用 | Undertakings (glyph 用 provisional). |

The rung lives in the HEADER, not a tab (F106/F116; §5.4).
...
- **The live scene owns the reveal; the log is the instant historical transcript** (F48) — painted behind
...
### 5.2 Perk cards (F56; taste.md 13)
...
pillars are **unnamed `--ink-faint` silhouette teasers** (D-055) — no kanji, no value. Estate shows its
...
is a player-TRIGGERED story beat** (D-110): the header surfaces a "ready to advance" summons that holds and
...
its card's center axis (F3). Repeatable actions pair a **`▶ auto` / `■ stop`** toggle (`aria-pressed`);
...
(§8). New Game returns to the zero state — Work tab, Story filter (F25).
...
(D-107/108/109): **COIN** — one `mon` 文 value in fixed mixed denominations mon → monme 匁 → ryō 両,
...
footer version (single-sourced from package.json, A21) opens it, and About deep-links the raw
...
Space is load-bearing (D-065/D-078/D-093): every labour and foe binds to a node; you WALK (`move_to`) to
work or fight; the deep satoyama 奥山 behind the danger ring gates the richer forage + the boar. Navigation
lives ONLY here (F107); Map ≠ Estate (F112); every visited location stays reachable (F113); NPC placement
shifts by tier/rung. **The node = two sections (F102/D-116):** a bordered "where you are now" description
...
(D-114): talk-to-reveal; the spectrum runs full-VN character → small person → tiny trader (straight to the
trade menu); vendors may be place-gated; every entity is discovered via a beat (F99). **Presentation is an
OPEN diverge (D-115)** — ~7 pure-CSS schematic variants (sumi roads, registration-mark nodes, `--shu`
...
lost fight can take; what you store, you keep** (a loss bleeds carried coin + rice + materials, D-113; koku
standing is never carried). Withdraw sits faded-with-reason when empty.

### 5.13 The full-screen dialogue (VN) scene (D-104/D-110)
...
| 6.7 First-ascension (once) | a held display-face title card + pillar silhouettes stirring + the audio swell; always lands BIG (D-062). |
...
| Pure #000/#FFF, raw white anywhere (DEV included) | Banned — warm washi + warm sumi only (F1). |
