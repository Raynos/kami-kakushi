...
(thresholds, costs, curves, conversion weights).
...
and **§2.4 (koku)**. This is the *only* moment with no nav, no tabs, no map — **one verb and a log.** You begin
...
> **T1** (§3.3.5); coin/*mon* (a **T2** reveal — there is **no *trade engine* in T0**, so **no TRADE strand at
> T0** either; a small capped **provisioning shop** where the player buys goods for his own character is a
> **personal koku sink** and is legitimate from T0); the inn & **rumours board** / folklore (**T2**, §2.13); the
...
> a managed unit. The kura is also the **storehouse / bank**: wealth splits **carried** (on your person, at risk
> to a lost fight's bite) vs **banked** (sheltered in the kura, safe). Once combat makes a loss bite carried
> wealth (R3), you **deposit / withdraw at the kura** — only while standing there — to shelter it; the balance
...
COMPLETES as a Phase-2 beat** (gated on the pillar-Influence floor, not the V-rungs; §2.17/§4.7.5). Coin (*mon*), the **component/quality**
