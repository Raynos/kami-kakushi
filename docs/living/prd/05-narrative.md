...
§5 owns the **story shape** of each tier — its diegetic beats. The curves and
thresholds it defers to live in §3 (reveal cadence) and §4 (numbers); the
combat-track, pillar, and gate mechanics are §2/§4's job.

The game runs across six tiers. **T0** is the tutorial (a showcase-in-miniature,
estate rungs R0→R7, the **Estate 家産** pillar only, the big first ascension);
**T1** is the full estate (R8→R15, **Arms 武威** reveals, E1→E2, the first paid
retinue, the sword line, the two-pillar gate that opens the valley); the Village
(Asagiri) is **T2**, the Region **T3**, the Castle-town **T4**, Edo **T5** — the
pillar-reveal ramp running **1→2→3→4→4**. The sword line opens at T1, the staff
...
T0 is the tutorial — a **showcase-in-miniature** that climbs the estate rungs
**R0→R7** and reveals only the **Estate 家産** pillar; the real 30-min-floor
estate grind is the full estate (T1), which climbs **R8→R15** and reveals **Arms
武威**. §5 owns the *story shape*; the rung-meter curve and gate thresholds defer
to §3/§4.

## T0.1 Overview

**Theme:** *earn your keep and a place at the table.* One declining hill estate — the Kurosawa *gōshi* house in
Asagiri valley — unlocked across a **small walkable map**. A half-drowned, nameless young man (~18–20),
pulled from the weir out of plain kindness, becomes "another mouth," then a reliable hand, then a hand who can
fight. The tier is the mediocre-start contract made playable: he is weak and slow, the first hoe blisters him,
...
> (§1.6.4/§4.0.1). Every tier is the **same two-phase motion**: **Phase 1** climbs the estate rungs via
...
> **estate-influence grind**. The tier's **pillar DEEDS accrue in Phase 2 and only there** (never
> across the rung climb — this blocks the "half the rungs, maxed deeds" exploit), and clearing the **hybrid
> pillar profile** is what tiers up. *(In **T0** only **one** pillar is revealed — Estate 家産 — so T0's Phase-2
> grind is **single-pillar**; the four-pillar version is what every later tier widens into.)* §5 fixes the
> *story* of this motion; the rung-meter curve, the curated per-rung activity sets, and the gate thresholds are
> §3/§4's job.

**Transition gate (T0→T1) — the two-phase shape.** **Phase 1** — climb the estate rungs **R0→R7** on the
**curated per-rung activities + the rung-meter + the story milestones** (an AND-gate; §4.1.1): survive the
humbling first fight, earn a sleeping-place and a name on the books, win the house's trust. Combat is **live**
this tier as an **activity** (it advances Combat Rank + the character level), but the **Arms pillar is NOT yet
revealed** — it surfaces only at the ascension and **names in T1**. **Phase 2** — the **capstone (R7)
rung OPENS the estate-influence grind**, where the tier's **single revealed pillar's DEEDS accrue** (the first
*shinden* recorded; the debt tranches chipped; the kura defended). Tier-up is the **HYBRID grade-gate scaled to
ONE open pillar — which collapses to EXCELLENT in Estate** (`1 EXC + 1 GRT + (N−2) GOOD` with
N=1). Meeting it only **unlocks the OPTION**; the tier-up is a **manual opt-in story event** the player triggers
(grinding Estate past the line earns a better, **grade-scaled boon**). Diegetically: the humbling first
fight survived; the first *shinden* recorded; the kura solvent enough that the immediate fires are out → the
estate trusts him with **its full body of work**, no longer in miniature — the **full estate (T1)** opens.
**Estate stage span:** E0 Foreclosure's Edge → E1 Stabilising (cross-ref §1.5.1).

> **The first ascension lands BIG (binding).** The **T0→T1 ascension is the first**, and it **always
> lands big on first contact regardless of grade**: a held **Yuji Syuku title card** brush-wipes in to name the
> new standing, the **locked pillar silhouettes stir** (the next, still-**unnamed** silhouette un-greys — T1
> names it **Arms 武威**), a **dream/mystery beat** fires (the §1.9 cadence; Origin stays dark), the audio
> **swells** (taiko roll + temple-bell 鈴, cross-ref `ui-design.md` §6.7 / `sfx-spec.md`), and the **grade-scaled
> boon** is revealed. Overshoot-grade *boon magnitude* still scales; the *ceremony* is grade-independent
> — the first is always big, earned exactly once.

**Felt arc:** cold-open helplessness (one verb, a near-bare screen) → the dignity of honest labour → the shock
of the first fight → the relief of being *named* on the household books → the swell of the **first ascension**.
Combat is **live this tier**, but it is
**INCREMENTAL** like everything else: its UI and systems surface **one beat at a time**, never an
...
3. **A season of honest work.** Sustained, reliable labour (farming → foraging → woodcutting → hauling) across
   ...
   identity clue — **no stat gift from the clue** (distinct from the optional T3 Origin accelerant).
...
5. **Begging for drills.** Kihei's creed, stated here and paid off at the finale: *"Talent is a story the lucky
   ...
   **first real trained weapon** (a **spear / *yari*** — the first of the two weapons beyond the improvised
   carrying-pole), **Equipment & Inventory**, the **Bestiary**, and the **bare auto-resolve loop + retreat**
   (character (combat) **level** begins, fed by combat-XP). Combat stats start near-zero. The rest of the combat
   surface **staggers in later, one beat at a time** (no UI-dump): the **graded durability bands** at R4 (with the
   loot→craft / Crafting loop), the **stance slot** at R5, and the **ability + item slots** at the first
   weapon-line **L10** milestone. T0's weapon roster is **three — the carrying-pole start + two more** (at least
   one craftable via the loot→craft loop; the full roster/params live in §2/§4), keeping the three-line
   archetype/signature vision.
6. **Earning the house's trust (indoor + the heir).** Win **Lady Chiyo's** regard for indoor work and heir
   ...
   **T2-V0** (where the allegiance tension actually goes live).
7. **Basic repairs, the capstone & the first ascension (the gate).** Drive the kura repair and complete the
   tutorial rung climb (**R0→R7**); the **capstone (bailiff, R7) rung OPENS Phase 2** — the **single-pillar
   estate-influence grind** — and the **House-Influence panel becomes visible**, un-greying the **active Estate
   家産 pillar** beside **locked, still-unnamed silhouettes**. In Phase 2 the **Estate DEEDS now accrue**:
   the first *shinden* reclamation recorded, the debt tranches chipped, the kura and grain-store held. (Combat is
   still an **activity** — kills→level, curated clears→Combat Rank — with **no Arms pillar to bank** yet; that
   reveals at the ascension and **names in T1**.) Clearing T0's **single-pillar gate — EXCELLENT in
   Estate** (the N=1 collapse of the scaled grade-gate) — edges the house off the foreclosure cliff. The
   player then triggers the **manual opt-in first ascension**, which **lands BIG on first contact** (the
   Yuji Syuku title card, the silhouettes stir, the dream beat, the music swell, the grade-scaled boon); Genemon,
   grudging, entrusts him with the estate's **full body of work** — the **full estate (T1)** opens (no longer in
   miniature). **Gate met.**
...
> pays off at T4/T5. **No** succession *beat* fires this tier — only the seed.

## T0.3 Antagonist arc — **The Inherited Debt** (the villain-less antagonist)

The T0 "antagonist" is **a circumstance, not a conspirator** — estate decline is a simple debt/misfortune,
**not conspiracy-linked**. It is personified only by a tired pawnbroker, **Moneylender
Tōkichi**, who is owed money and is not cruel about it. The debt is **introduced** here (the leaning gate, the
red ledger); its economic **resolution plays out across the T1 full-estate grind** — the TREASURY lane chips it
toward solvency (continued at **T1.3**).
...
- **Resolution (purely economic, multi-route):** **Peaceful** — out-grind the interest and renegotiate.
  ...
  **restructured, not erased**; it becomes the **root-sin the house later atones for at Kuzuhara** (T3) —
  emotional payload, **no antagonist.** *(The Peaceful/Assertive choices are authored as **intra-line dialogue
  choices** — data, not scripting; deterministic; only chosen-flags persist. The data shape is §2/§6's job;
  §5 names only the routes.)*
- **[MOTIF: rigged box] — explicitly ABSENT.** The racket has **no fingerprints on T0.** Sadamune's
...
**Sōan**. *(The teen field-lad is **Heita**. The porter's-knot beat is delivered by the groom **Sota**.)*
...
- **Naoyuki** — the in-house **rival** (talent-foil, *converted not innate*). Voice: bored brilliance, chafing
  ...
  earned **grudging-respect bridge** fires in T2.2; the ally-flip pays off at G5/T3.3).
...
- **[THREAD: Tama] estate-side (SEED ONLY):** in T0 the house simply sees a hand who earns his place —
  **nobody speaks the name "Tama."** The legend **ignites in T2** (Sayo, V0); the estate's plain baseline then
  reads as the counterweight to the village's certainty (paid off T2–T3). Keep it foreshadow-only in T0
  (the dream + the porter's-knot); do not stage a Tama "debate" before the village raises it.

## T0.6 Areas & mob roster

**Areas (T0 estate + the shared near-wilderness ring — navigable nodes on the **small walkable map**,
grown further in T1; cross-ref §1.7):**
...
- **The Deep Satoyama (奥山)** (shared wilderness, one hill past the near satoyama — behind the danger ring, at a
  higher satiety cost) — the richest forage (deep-woods *sansai*) and the **boar's den**; the deeper second ring
  of the danger gradient, and the load-bearing node whose walk earns the game's best T0 yield.

**Mob roster (grounded; part of the ~5-mob set; conditioning-gated; cross-ref §1.13):**

| Mob | Type | Where | Role |
|---|---|---|---|
| **Wild boar (*inoshishi*)** | animal | deep satoyama (奥山) | The first real grindable threat after the humbling fight; dens in the deep woods and raids down to trample crops. Drops boar-hide (armour craft) + meat. The "something's in the lower field" pest-control intro that tracks up to its den. |
| **Crop-raiding monkeys (*saru*) troop** | animal | home paddies / field-edge | Fast, evasive pack nuisance raiding the home paddies; teaches multi-target combat & evasion. Low danger, high annoyance. |
...
> "yokai" arrives only via the inn rumours board (T2+) as INVESTIGATE-then-confront one-shots.
...
**There is NO fixed quest-type budget** — author whatever quests fit each stage; the handful below is just
...
- **The porter's-knot beat** — "you've hauled before." **[THREAD: Origin]** — **no stat gift from the clue**
  (identity flavour only; distinct from the optional T3 Origin accelerant).
...
# T1 — Estate (the full estate)

## T1.1 Overview

**Theme:** *the tutorial's over — the real grind.* The estate is no longer in miniature: its household, land,
debt, and arms all **scale into real depth**, and the mediocre-start protagonist becomes a genuinely
**capable estate man**. A fresh stretch of the home ladder is climbed — **R8→R15** (≈8 new rungs, ~16 estate
rungs in all), still **estate-scale, before any village**: **Kura Warden → Field Reeve → Drill-yard Hand →
Stable & Woodlot Master → Ledger-hand → Armsman of the House → Under-steward → Trusted Man of the House**
(placeholders; §3). This is where the **≥30-min/rung floor BINDS for the first time** (T0
was floor-exempt); Phase-1 runs roughly **~5–8h** (§4.8 owns the figure). Difficulty
stays **humbling THROUGHOUT** within guardrails. The estate's domain is still **its own land** — the
valley does **NOT** open until the Arms ascension fires at tier-end.

> **Progression shape (binding) — climb the rungs (Phase 1), then the estate-influence grind (Phase 2)**
> (§1.6.4/§4.0.1). T1 runs the same two-phase motion on a **two-track rung-meter**: Phase 1 climbs R8→R15 on
> an **Estate-Service** (labour) sub-meter **and** a parallel **Combat-Rank** (martial) sub-meter, promoting on
> the **AND-gate** (both sub-meters ≥ floor **and** the rung's story milestone). **Phase 2** opens **only after
> the R15 capstone** as the **two-pillar (Estate + Arms) influence grind**, where the tier's **pillar DEEDS
> accrue** (never across the rung climb). Clearing T1's **hybrid pillar profile** tiers up to the Village (T2).

**Transition gate (T1→T2) — the two-phase shape.** **Phase 1** — climb **R8→R15** (the full-estate
ladder): the debt chipped toward solvency, the *shinden* reclaimed, the drill yard trained into a discipline,
the grain-store and roads held. **Phase 2** — the **R15 capstone (Trusted Man of the House) OPENS the two-pillar
influence grind**, where the tier's **pillar DEEDS accrue** across **Estate 家産 + Arms 武威**. Tier-up is the
**HYBRID two-pillar gate**: **exactly 1 GREAT + 1 EXCELLENT** across Estate + Arms, **both ≥
GOOD**, **NO overflow** (a surplus in one pillar can never cover a deficit in the other). Meeting it **unlocks
...
arrive; the Arms ascension lifts the lord's horizon **past his own gate** for the first time.

## T1.2 Main-quest beats (toward the T1→T2 gate)

The full-estate ladder (LOCKED shape at §1.5.1; rung copy in §3): **Kura Warden → Field Reeve → Drill-yard Hand
→ Stable & Woodlot Master → Ledger-hand → Armsman of the House → Under-steward → Trusted Man of the House** —
every rung a role **within the house's theme**, climbed on the **two-track rung-meter** (Estate-Service +
Combat-Rank, AND-gated). The tier's **pillar DEEDS are gated to Phase 2** (post-R15); combat clears advance
...
3. **Chipping the debt (TREASURY).** Sit with clerk **Tanomo** and grind the **Inherited Debt** (Tōkichi's loan)
   toward **solvency**; *goyōkin* leverage opens. The circumstance — **not a villain** — slowly lifts (the
   "rigged box" racket still has **no fingerprints on the estate**; it first surfaces at T2). *(See T1.3.)*
4. **The drill yard becomes a discipline (Combat-Rank) — the Sword line opens.** Drillmaster **Kihei** turns the
   humbling first fight into real training — curated forms/sparring/conditioning that build the **Combat-Rank**
   sub-meter. The **Sword line (Line 2) opens** here — a whole second weapon family **found or crafted, never
   gifted** (the quick *Kodachi*, the Iai-draw *Uchigatana*, the heavy *Ōdachi*; T1 grows the roster **+3**),
   taken into the **Near Satoyama** against boar, monkey-troop, the **wolf-pack** and the *mamushi*.
5. **The Arms 武威 pillar reveals (Armsman of the House).** Kihei **names** the pillar un-greyed at the
   ascension: what counts as a martial **DEED** (declare a road safe, clear a nest, defend the grain-store) and
   that the season's **security is judged** on a new high-water. The pillar is real even before it pays out — but
...
7. **The house turns solvent & the estate mends.** The TREASURY lane crosses **debt→solvency**; LAND matures
   ...
   danger ring now patrolled. The long shadow of grandfather **Sadamune's** failed flood-venture (Dowager
   **Toku's** grief) lifts.
8. **Trusted Man of the House (R15) — the capstone, the two-pillar grind & the Arms ascension (the gate).** Reach
   the top estate rung; the **R15 capstone OPENS Phase 2** — the **Estate + Arms** influence grind, where both
   pillars' **DEEDS finally accrue** (**70/30** deeds/seasonal). Clearing the **two-pillar hybrid gate** (1 GREAT
   + 1 EXCELLENT, no overflow) unlocks the option; the player triggers the **manual opt-in Arms ascension** — the
   **next (3rd, still-unnamed) silhouette stirs** (T2 names it **Office 官威**), a **dream/mystery beat**
   lands (the §1.9 cadence; Origin stays dark), and the **valley comes into view past the estate gate** (the **T2
   teaser** shown at the ascension — **not** playable T1 content). The estate trusts him to carry its business
   beyond its own land — the **Village (T2)** opens. **Gate met.**

> **Succession seed (continued).** Lord **Shigemasa**'s weariness and Naoyuki's restless coasting keep building
> through T1 dialogue (seeded T0) so the generational arc reads as earned when it pays off at T4/T5. **No**
> succession *beat* fires yet — only the deepening seed.

## T1.3 Antagonist arc — **The Inherited Debt** (continued; the villain-less circumstance, resolved)

The T1 antagonist is the **same circumstance** introduced at **T0.3** — the compound interest on Sadamune's
failed flood-venture — now **fought and resolved** by the full-estate grind (a circumstance, **not** a
conspirator). It stays personified only by the tired pawnbroker **Tōkichi**.

- **What it blocks:** the house cannot reach beyond its gate while it is insolvent; the debt is the weight on
  every ledger.
- **Resolution (purely economic, multi-route):** **Peaceful** — out-grind the interest through the **TREASURY**
  lane and renegotiate. **Assertive** — refuse the worst terms and force a fair table by **proving solvency** (no
  fight). The debt is **restructured, not erased**; it becomes the **root-sin the house later atones for at
  Kuzuhara** (T3) — emotional payload, **no antagonist**. *(Routes authored as intra-line dialogue choices —
  deterministic; only chosen-flags persist; the data shape is §2/§6's job.)*
- **[MOTIF: rigged box] — still ABSENT.** The racket has **no fingerprints on the estate** through T1; it first
  surfaces at the village (T2 — Magobei's doctored *masu*).

## T1.4 Estate roster & buildings growing (E1 → E2 Recovering)

**Joins this tier:** men-at-arms **Gohei & Yatarō** (ex-*ashigaru* hired off the road — the **first paid
outsiders**, the 2–3-man rota at E2); a charcoal/smith-adjacent artisan; maidservant expansion under **Oai**.
*(Green recruit **Tokujirō** arrived at the E1 seam at the close of T0 — the mirror who later proves the MC's
growth.)*

**Buildings (E1 → E2):**
- **E2 — Recovering:** a proper granary raised; two estate workshops (textile + charcoal/smith-adjacent); a
  repaired forecourt and a real (if low) palisade fence; 2–3 men-at-arms on a rota; a managed woodlot — the
  visible mending of a house back on its feet.

> **Build/recruit framing (binding):** every build is a diegetic beat ("the frame is raised"), every recruit a
> light roster card — **NOT a management minigame** (no labour-gang to assign, no managed sub-economy; §4.7.5).
> Estate physical growth is **flavour you can see**, not a sim. *(The heavier prosperity build — E2→E3 — lands
> at the Region, T3.)*
...
- **Kihei** — the **arms/combat mentor**: turns the humbling fight into discipline, opens the sword line,
  ...
  you are not lucky, so you will work"* — is drilled until it holds.
- **Tanomo** — the clerk at the ledger; the face of the TREASURY debt→solvency grind.
- **Dowager Toku** — backstory keeper; **Sadamune's** flood-venture and the family shame the debt encodes.
- **Naoyuki** (heir, estate-side) — still the in-house **rival** (talent-foil, *converted not innate*); the
  **grudging-respect bridge** is deepened here but **fires at the Village (T2.2)** when a shared danger throws
  them together — the earned bridge from the T0 vouch to the **G5 ally-flip** (T3.3). **[THREAD: Naoyuki]**
- **[THREAD: Origin] this tier:** the dream keeps a guaranteed cadence (sensory fragments; the name surfacing at
  the edge of sleep) but the thread stays **dark/foreshadowed** — the Origin faction does not *open* until T3.

## T1.6 Areas & mob roster

**Areas (T1 estate, the walkable map grown; cross-ref §1.7):** the T0 estate nodes (kura, forecourt, paddies,
stables & woodlot, drill yard, *omoya*) **deepen**, and the small walkable map **grows outward to the Near
Satoyama** danger ring now patrolled (foraging groves, bamboo stand, the charcoal-burners' edge) —
reveal-per-beat. The valley itself stays **locked** until the Arms ascension (the T2 teaser shows it at tier-end).

**Mob roster (grounded; the ~5-mob set deepens; conditioning-gated; cross-ref §1.13):** the T0 set returns at
weight — **wild boar**, **monkey-troop**, **rice-rats & *mamushi*** — joined by the **giant-hornet
(*suzumebachi*) nest** (a CLEAR-the-nest AoE beat near foraging) and the **wolf-pack (*ōkami*)** in the foothills
(the first fight that punishes a lone, under-geared player). The bestiary stays **GROUNDED estate +
near-satoyama beasts** — the **first HUMAN threat (bandits / starving deserters, *nobushi*) waits for the Village
(T2)**.

> **FOUND-gear & the growing roster.** The Near Satoyama's beasts feed the **loot→craft loop**; this is
> how the **growing weapon roster** surfaces diegetically — T1 grows it **+3** and opens the **2nd weapon LINE (a
> sword)** — worn or rough blades found and refitted, never gifted. §5 keeps the FOUND-gear flavour; the
> roster/archetype params live in §2/§4, the reveal cadence in §3.

## T1.7 Open-ended side-quests (T1)

All optional; none gate the spine. **There is NO fixed quest-type budget** — the **PEST-CONTROL / HUNT /
CLEAR / DEFEND** set deepens here on estate ground; author whatever quests fit the full estate. *(No inn-rumour
folklore yet — the rumours board is a **T2** unlock; T1 keeps to the estate and its near-wilderness.)*

- **The deeper *shinden* reclamation** — a multi-stage LAND project that pays out new paddy (warms the flywheel;
  never gates the spine).
- **The debt renegotiation** — force Tōkichi to a fair table by proving solvency (the TREASURY payoff;
  multi-route). **[THREAD: Inherited Debt]**
- **The wolf-pack on the foothill trail (DEFEND/HUNT)** — a lone, under-geared clear that drives the need for the
  sword line + better gear.
- **The hornet's nest at the woodlot edge (CLEAR)** — an AoE conditioning test near the foraging grounds.
- **Drilling Tokujirō from zero (seed)** — the green recruit's first lessons foreshadow the "you now teach others
  from zero" layer that re-homes much later. **[THREAD: Tokujirō — seed]**

---

# T2 — The Valley (the estate's domain expands)
...
> **Progression shape (binding) — climb the rungs (Phase 1), then the estate-influence grind (Phase 2)**
> (§1.6.4/§4.0.1). T2 repeats the two-phase motion on a fresh ladder: **Phase 1** climbs **V0→V7** on the
...
**Two parallel tracks:** the **estate ladder is the spine** (its milestones gate T3). Running alongside it is the
...
**fully completable** and an **optional accelerant** (~10–15% faster to T3), but **NEVER gates the spine**
(§1.5.4, §2.15). The inn's **rumours board** begins delivering optional light folklore on the side-track.

**Transition gate (T2→T3) — the two-phase shape.** **Phase 1** — climb **V0→V7** (*"clean your room"*):
...
with more sway to surpass). Tier-up is the **HYBRID tier-gate** across T2's **three revealed pillars** —
**good in ALL three** (Arms + Estate + **Office**), **great in 2–3**, **excellent in 1–2**, **no overflow**.
Diegetically: errand-authority earned; office in valley affairs held; cash-crops online.
**Estate stage:** **E2 Recovering (held)** — E1→E2 landed in T1; the E3 Prosperous payoff lands at the Region
(T3.4).
...
> **What's net-new at the Village.** This is the **first tier OUT** (the Arms ascension just
> fired and the estate gate opened onto Asagiri). New here: the **Office 官威** pillar **reveals** (V4); the
> **3rd weapon LINE — staff / polearm — opens** (PULLED FORWARD from the Region per §3, so the roster is
> **complete by end-of-T2**); **rival houses Tomita (money) & Akagi (honour) are INTRODUCED and the contest
> BEGINS** (the climax — the G5 Naoyuki ally-flip, the G7 dethroning — is **held for the Region, T3**);
> **cross-pillar combos surface as the PARTIAL Office-pairs set** (the full 4-pillar set waits for the Region).
> The retinue **Gohei & Yatarō** (hired back at the estate in **T1**) is **deployed and expanded** here. The
> first **HUMAN** threat (bandits / *nobushi*) arrives. The estate **holds at E2 Recovering** (E1→E2 landed in
> T1; the E3 payoff lands at the Region, T3).
...
2. **Becoming a recognised hand of the house (V1).** The valley starts knowing the house's man — headman
   ...
   **character (combat) level** via kills; the **Arms PILLAR deeds** are reserved for the Phase-2 estate-grind.
   The inn & rumours board unlock (folklore begins, optional side-track).
...
4. **Running the valley economy for the house (V3).** Bring the valley economy and the estate's cash-crops to a
   recorded seasonal result — the **real village silk market** + the **silk / sericulture *meibutsu*** sub-engine
   open under weaver **Onatsu** (LOCKED; trade hard-capped **≤⅓** of Estate & Wealth, the clamp now genuinely
   exercised). *(This **matures** the thin TRADE taste that began on the estate at T1 — T1 = estate-internal
   surplus + a tiny-market taste; T2 = the real village market + broker standing; never T0.)* Broker meters
   appear. The estate's domain now reaches the valley's livelihood.
5. **Trusted of the headman (V4) — the skim surfaces.** Put a valley fire out on the estate's behalf: **Foreman
   Magobei's** rice-skim surfaces here (the T2 antagonist; see T2.3). The house earns its **first Office
   standing** (the **Office pillar reveals at T2**); Yagōemon's *personal* regard rises on the side-track.
6. **Sworn man-at-arms of the house (V5).** Stand a real watch for the valley **in the house's name**; survive
   the first dangerous-road encounter; the retinue **Gohei & Yatarō** (hired back at the estate in **T1**) is
   now **deployed and expanded** with valley men — an **expansion, not a "first."** The estate becomes the
   valley's shield.
6a. **Shoulder to shoulder with Naoyuki (rivalry → grudging respect). [THREAD: Naoyuki]** A shared danger —
   ...
   rivalry seed and the G5 rival→ally flip** (T3.3) so the conversion reads as **earned**, not abrupt.
7. **Right-hand-in-waiting (V6) — agent over the valley; the rivals appear.** The lord first believes the house's
   impact beyond the estate is possible; "clean your room" nearly done; the alliance/standing levers that point
   the house at the region appear. **Rival houses Tomita (= money) & Akagi (= honour) are INTRODUCED and the
   contest BEGINS** — the first **contested** deed lands; their per-pillar standing is **real, not scripted**
   (light rival stats, an overtake the climb *computes*, §1.11). **Cross-pillar combos surface as the PARTIAL
   Office-pairs set** (Arms×Office, Estate×Office — the full 4-pillar set waits for the Region/T3, when **Name**
   reveals); combos stay **post-trade-clamp** and **excluded from the gate-check AND the trade-⅓ denominator**
   (§4.3.1). The **climax** — the G5 Naoyuki ally-flip, the G7 dethroning — is **held for the Region (T3).**
8. **Agent of the house, the valley anchored (V7) — the capstone & the Phase-2 grind (the gate).** Estate
   ...
   pillars** (Arms + Estate + Office) — where the tier's **pillar DEEDS accrue**; clearing T2's **hybrid profile**
   (good in all three; great in 2–3; excellent in 1–2) opens the **region** map and the **T2→T3** quest (the
   domain expands again, to the Region); the **rivals already in contest (Tomita & Akagi) array on the Region
   board** — the climax (G5 ally-flip, G7 dethroning) comes at T3. **Gate met.**

> **Allegiance, live this tier. [THREAD: Tama]** The continuous Tama-vs-farmhand leaning (§1.5.4) is
...
> antagonist choices in **T0.3 / T1.3 / T2.3 / T3.3** (Peaceful / Assertive / Partial) are authored as **intra-line
> dialogue choices** — data, not scripting; deterministic; only chosen-flags persist. §5 names the leans; the
> data shape is §2/§6's job.)*

## T2.3 Antagonist arc — **Foreman Magobei**, the "tanuki of the rumours board"

The first small **human** antagonist. He skims village rice with a doctored measuring-box
(***kyō-masu***), keeping stores short and stealing the surplus the T2→T3 gate needs.
...
## T2.4 Estate roster & buildings (E2 Recovering — held; village-seconded faces)

**Joins this tier:** a carpenter-apprentice seconded via village **Carpenter Risuke**; a charcoal artisan;
maidservant expansion under Oai. *(Men-at-arms **Gohei & Yatarō** were hired back on the home estate in **T1**;
here they are **deployed and expanded** with valley men — an expansion, not a "first.")*

**Buildings:** the **E2 Recovering** fabric — a proper granary, two estate workshops, the repaired forecourt and
low palisade, the men-at-arms rota, the managed woodlot — was **raised on the home estate in T1**; at the Village
the estate **consolidates at E2** while the valley starts **crediting the Kurosawa's hand** again. The heavier
prosperity build pays out later at **E3 Prosperous (T3.4).**

> **Village cast stays static (binding).** Estate growth pulls *seconded/recruited* faces (Risuke's apprentice,
> the deployed men-at-arms); the village's own reputation-web cast does **not** balloon.
...
- **Naoyuki** (heir; estate-side) — the in-house rival begins his **rivalry → grudging respect** turn this tier
  (the interstitial beat in T2.2/6a): fighting a shared danger beside the MC, he acknowledges him for the first
  time — the earned bridge from the T0 vouch to the **G5 ally-flip** against Tomita. **[THREAD: Naoyuki]**
...
an unnamed post-town. The thread stays **dark/foreshadowed** — the Origin faction does not *open* until T3.

## T2.6 Areas & mob roster

**Areas (T2 village + deeper shared wilderness; cross-ref §1.7):**
...
- **The Jizō at the Weir** — the boundary *jizō* **at the weir where he was found** — **the single find-spot**:
  the one place "where he was pulled from the river." It is also the **ONE** capped residual-ambiguity
...
- **The River, Ford & Weir** (shared wilderness) — the "kappa" spot; fishing. *(The find-spot — the **jizō at
  the weir** — is its own row above; "where he was found" is a **single co-located place**, and the lone
  residual-ambiguity beat is co-located there.)*
...
**Mob roster (grounded; completes the ~5-mob set; conditioning-gated):**
...
*(A rogue bear is available as an optional HUNT quarry; see §T2.7.)*

> **FOUND-gear & the growing roster.** The valley's human threats drop **worn FOUND gear** that feeds
> the loot→craft loop; this is also how the **growing weapon roster** surfaces diegetically — T2 grows the roster
> **+4** and opens the **3rd weapon LINE — staff / polearm (Bō・Naginata・Kanabō・Tetsubō)**, **PULLED FORWARD
> from the Region into the Village** (§3), found and refitted rather than gifted — so the **roster is
> complete by end-of-T2** (the spear L1 / sword L2 lines, opened at T0 / T1, also deepen via their L25/L50
> signatures). The **Region (T3) is then combat DEPTH — no new line.** §5 keeps only the FOUND-gear flavour; the
> full roster/archetype params live in §2/§4 and the reveal cadence in §3.

## T2.7 Open-ended side-quests (T2)

All optional; none gate the spine. **There is NO fixed quest-type budget** — the **PEST-CONTROL / HUNT /
CLEAR / DEFEND** set is just T0–T2's *starting* set, not a cap; author whatever quests fit each stage, with
...
- **The inn rumours-board opener — the "kappa" of the ford.** **[FOLKLORE → cause]** an undertow + an undercut
  ...
  uneasy.
- **Magobei's skim — the "tanuki."** **[FOLKLORE → cause]** = the skimming foreman (ties into T2.3).
...
- **The weir-jizō offering** — the **one residual-ambiguity beat** (an offering by an unknown hand, left at the
  co-located **jizō at the weir** find-spot). It is *not* resolved; it is the single capped token (§1.13).
...
- **The "fox-fire" ridge (seed).** **[FOLKLORE → cause]** first *rumour* of ghost-lights on the ridge; the full
  INVESTIGATE-then-confront (a hidden charcoal kiln) resolves at **T3** when conditioning lets the MC range
  there.
- **A weaver's missing dye-lot (open-ended help).** Onatsu's silk run comes up short a bolt; figure out where it
  ...
  *meibutsu* sub-engine without gating it. *(A representative example; there is no fixed quest set.)*
- **The night-heron of the paddies (rumour).** **[FOLKLORE → cause]** a "ghost-bird" haunting the flooded
  fields = an ordinary heron; a one-shot INVESTIGATE that pays a little foraging lore. *(A representative
  example; there is no fixed quest set.)*
- **Hard times down the road (era texture — Fork A2).** One or two optional rumour-board tidbits gesture at
  **lean years stirring elsewhere** — a pedlar out of the lowlands says the rice was thin two districts over,
...
> |---|---|---|
> | **Kamikakushi** (master lie) | A landslide victim swept into the river (Tahei); a child who *ran* (Tama/Otsuru); negligence dressed as fate (Kuzuhara) | spans T0→T3 |
> | Weir/ford **kappa** | Undertow + undercut bank; later a poacher's weighted nets / smugglers' sinking-spot | T2 ford |
> | **Tanuki** stealing rice | Magobei's skim | T2 village |
> | **Fox-fire** on the ridge | A charcoal-burner's hidden kiln | seeded T2, resolved T3 |
> | Yearly **soul-calling rite** | Grief-work that nonetheless yields hard evidence (Ryōa's register) | T2 shrine |
> | The unidentified-hand **weir-jizō offering** | *(left ambiguous — the ONE capped token)* | T2 weir-jizō find-spot |

---

# T3 — Region (the personal-mystery payoff lands here)

## T3.1 Overview

**Theme:** *the estate's domain expands to lead a region.* The HOUSE rises — a third fresh **estate rank
ladder** is minted (`G0→G7`, region-scale, ~8 rungs; see T3.2) on which the Kurosawa's standing climbs
...
threads PAY OFF** — but on **two different rails**: the **lost-child TRUTH (Otsuru)** resolves on
...
> **Progression shape (binding) — climb the rungs (Phase 1), then the estate-influence grind (Phase 2)**
> (§1.6.4/§4.0.1). T3 repeats the two-phase motion: **Phase 1** climbs **G0→G7** on the **curated per-rung
> activities + the rung-meter + story** (AND-gate); **Phase 2** opens **only after G7** as the
> **estate-influence / four-pillar grind**, where the tier's **pillar DEEDS accrue** (never across the rung
> climb). Clearing T3's **hybrid pillar profile** tiers up to T4.

**Transition gate (T3→T4) — the two-phase shape.** **Phase 1** — climb **G0→G7**: *win the region* (the
rival houses are no longer the leaders). **Phase 2** — the **G7 capstone OPENS the estate-influence / four-pillar
grind**, where the tier's **pillar DEEDS accrue**, and clearing T3's **HYBRID tier-gate** across the **revealed
pillars** (4 — Estate + Office + Arms, with **Name** surfacing this tier) — **good in ALL revealed pillars**,
**great in 2–3**, **excellent in 1–2**, **no overflow** — makes the castle-town rulers **confer regional
...
so T4/T5 carry the house's power rise alone.

## T3.2 Main-quest beats (toward the T3→T4 gate)
...
Phase 2** (post-G7). Labour and combat interleave throughout — combat clears advance **Combat Rank** (the
...
3. **The post-town opens (broker of the post-town trade). [THREAD: Origin] — the faction opens.** Once the
   ...
   estate's mon; the regional broker runway begins. The **origin reunions** start here (see T3.5).
   *(**G2 name-softening:** the **spine-guaranteed** G2 event-log line must **not** speak the
...
5. **Kuzuhara — the house's root-sin (recognised regional retainer).** With conditioning + standing, the MC
   ...
   **root-sin** (Sadamune's neglected flood-works, atoned for here). A multi-stage **river-works** project
   secures the valley; resettlement re-founds the struggling hamlet as a T3 region project; **the drowned are
...
6. **The road-security captain (captain of the detail).** Break the brigand roost; secure the trade pass with a
   **hard-capped 2–3-man detail**. The **Hanzaki** encounters escalate (survived, not won; see T3.3). A
...
7. **The lost-child truth resolves (alliance-broker, G6). [THREAD: Tama] — SPINE PAYOFF (guaranteed).** Tracking
   ...
   town of **Yanagi-watari** — a **different T3 locale** from the origin-reunion hub Sawatari-juku, so the two
   personal threads land in separate places. **The truth:** Tama was a **girl**
   ...
   truth is SPINE-GUARANTEED at G6 for every player** (§1.4). **[THREAD: Origin]** runs *alongside* this span
   ...
   reclaiming the name** (the name-reclaim is never a spine beat).
8. **Leading house of the region (the capstone & the Phase-2 grind — the gate).** The rivals are no longer the
   ...
   clearing T3's **hybrid profile** makes the castle-town rulers confer regional leadership and invite the house
   in. **Gate met.**

> **Origin track & the missable name-reclaim (binding note; one place).** The climax structure,
...
> **CONDITIONAL** on completing Origin O5 (see T5.5).

> **The dream rule (binding) at payoff.** The returning **memory / backstory reveal** grants
...
## T3.3 Antagonist arc — **Rival House Tomita** (spine) + **Rival House Akagi** (foil), with **Hanzaki** as the road's teeth

The T3 antagonist is the **two rival houses** (exactly two) — the region's **incumbent samurai
houses** when the Kurosawa arrive — with the scarred *rōnin* **Hanzaki** as the dangerous combat beat. **Tomita
(= money) & Akagi (= honour) were INTRODUCED and the contest BEGAN back at the Village (T2.2/T2.7 — §3);
the Region is the contest's CLIMAX** (the G5 Naoyuki ally-flip and the G7 dethroning land here). They are the
**contest layer that rides the spine**: the "win the region" gate (G7) *is* surpassing them. **Both are
...
- **Resolution (multi-route):** **Peaceful/commercial** — out-supply and out-arbitrate the valleys; settle
  **Akagi** by *restoring its honour* (give the proud old line its precedence back); out-leverage Tomita's
  ...
  other** (money vs honour). *(Routes are authored as intra-line dialogue choices; cross-ref the T2.2 allegiance
  note.)*
- **Talent-foil rule (binding):** every Tomita/Hanzaki advantage is shown as **bought, lucked, or trained** —
  ...
  rival into **ally against Tomita** — the **G5 flip** the T2.2/6a interstitial earned).
- **Rival standing is real, not scripted (binding).** **Tomita** and **Akagi** (plus a few
...
## T3.4 Estate roster & buildings growing (E2 → E3 Prosperous)
...
> at the Region; stages **E4–E5** (fortified seat → restored-and-surpassed) belong to the castle-town and capital
> chapters (T4/T5).

## T3.5 Key NPC & dialogue beats

**Origin (opens at T3; Sawatari-juku & Kuzuhara; cross-ref §1.8). [THREAD: Origin] — payoff cast.** *(Binding:
this whole cast is the **OPTIONAL, MISSABLE** Origin O5 side-track per the T3.2 note; the **Tahei name-reclaim**
is its capstone, **not** a spine beat.)*
- **Jinpachi** (father, ~50; a senior porter long away on a far stretch of the *kaidō* — **known alive but
  long estranged** from the household). The source of the porter's-knot lineage (**no retroactive gift from the
  reveal**). **His reconciliation resolves at T3** alongside the rest of the family, with an **optional later
  emotional callback at T5** (the recovered family proud behind the MC). *(Deliberately **NOT a second
  resurrection**: **Oyuki grieved only
...
- **Osen** (a sweetheart half-remembered, ~17) — optional, gentle, **narrative-only** relationship thread the
  dream surfaces (no dating-sim). *(Kept clearly distinct from the lost-child **Otsuru**.)*
- **Otsuru** (the real Tama, alive & grown **down-valley in Yanagi-watari** — a river-crossing market town
  kept **separate** from the origin-reunion hub Sawatari-juku) — mystery payoff & living
  proof; **costly & incomplete** reunion (she may not forgive). *(Distinct from the sweetheart **Osen**; keep
  voices/roles distinct so they don't blur. Otsuru's truth is the **spine** G6 payoff; the rest of this cast is
  the **optional** Origin side-track.)*
...
Kuzuhara the house **atoning for its own root-sin.** *(No "Kuzuhara survivor" / co-investigator character: the
MC has **no personal tie to Kuzuhara** — it is purely a struggling, depopulated upstream hamlet the
player can help resettle as a T3 region project. The MC's origin is uncovered via the **dream → the
Sawatari-juku family** (Oyuki / Jinpachi / etc.), never via anyone here who recognizes him.)*

## T3.6 Areas & mob roster

**Areas (T3 region; the region cut-set; cross-ref §1.7 / §1.7.1):**
...
- **Sawatari-juku & the wider post-town region** *(mixed)* — the origin reunion hub (optional — **Tahei's
  living family**) + the *toiya* transport office / waystation trade layer (the practical surplus-export runway to T4).
- **Yanagi-watari — down-valley river-crossing market town** *(spine, personal-mystery)* — a second post-region
  ...
  found (the **G6 spine TRUTH**), kept **separate** from the origin-reunion hub so the two T3 threads don't share
  one locale.
- **The Kaidō Porters' & Transport Guild** *(T3 trade-backbone)* — routes, *sekisho* pass-tiers, route-risk; the
  trade backbone (met via Kanta's first favour run). **This is a T3 node, NOT the T4 cliff-hanger** — the T4
  cliff-hanger is the **castle-town / Daikan first-contact** (see the T4 header). The porter/transport guild
...
**Mob roster (grounded; region-scale; reuses the set + adds human threats; conditioning-gated):**
...
> **FOUND-blades & combat DEPTH — no new line (§3).** The region's better-trained human threats
> drop better **FOUND blades** (a worn *kodachi*, a fitted *yari*; Hanzaki's gear a late prize) that **deepen the
> already-complete roster** — but **T3 opens NO new weapon line.** The roster (spear L1 / sword L2 / staff L3) is
> **complete by end-of-T2** (the staff line was pulled forward into the Village); the Region is **combat DEPTH** —
> the existing lines mature via their high-level (L25/L50+) signatures. §5 keeps the FOUND-gear flavour; the
> roster/params live in §2/§4, the reveal cadence in §3.
...
## T3.7 Open-ended side-quests (T3)

All optional; none gate the spine. **There is NO fixed quest-type budget** — author whatever quests fit
...
- **The origin reunions.** **[THREAD: Origin]** Jinpachi, Oyuki, Okimi, Denbei, Kanta, Osen — the rungs of the
  **OPTIONAL, MISSABLE** Origin one-tier rep side-track (`O0→O5`, §3.6.2). **The Tahei name-reclaim lands HERE
  (at O5), not on the spine.** The **reveal carries no retroactive gift**; the **track** is the optional
...
- **The fox-fire ridge.** **[FOLKLORE → cause]** = a hidden charcoal kiln (resolves the T2 seed). One-shot.
...
- **The *toiya*'s misrouted manifest (open-ended help).** A consignment goes astray on the *kaidō*; trace it
  ...
  *(A representative example; there is no fixed quest set.)*
...
> **Belief→cause additions (T3; binding).** "One-eyed mountain god" → Hanzaki + terrain. "Fox-fire ridge" →
...
> ambiguous token remains the Asagiri **weir-jizō offering**; **no new ambiguity beats** are added at T3.

---

# T4 — Castle-town (the closing cliff-hanger)

> **The game closes on a single castle-town cliff-hanger — the castle-town / Daikan first-contact.** After the
> T3 capstone a **sealed summons** delivers the Kurosawa's representative to **present credentials at the
> Daikan's Office** (one node — the receiving anteroom — one verb: present credentials), followed by a **bare
> rented castle-town townhouse beachhead** — a single room with a gate that **mirrors the T0 kura cold-open**,
> re-running the fractal-incremental motion at a new frontier with the least new art/systems. The T3 capstone
> opens onto exactly this one castle-town teaser, then ends. **The rest of T4 below is the castle-town chapter's
> forward arc — the designed continuation the cliff-hanger points toward.**

## T4.1 Overview

**Theme:** *be reckoned with by the people who actually rule* — the castle-town, the *daikan*/*tedai*,
inter-*han* markets. The takeover is won **socially** — multi-route (office / economy / marriage /
out-maneuvering rivals) plus assertive martial-security leverage; **never open rebellion**. "Take over" =
becoming the **dominant house holding key domain offices.**

**Transition gate (T4→T5):** a **"taste of Edo"** — the house is **called to staff & run the *domain's* Edo
...
**Estate stage span:** E3 Prosperous → E4 Fortified Seat *(E3 is attained at the Region; this chapter carries
the estate from E3 upward).*

## T4.2 Main-quest beats

*(The cliff-hanger the game closes on is beats 1–2 — the summons + the townhouse beachhead, mirroring the T0
kura cold-open; beats 3–7 are the designed continuation the cliff-hanger opens onto.)*

1. **The summons.** After the T3→T4 service is rendered, a low *tedai* delivers a sealed note: present the
...
## T4.3 Antagonist arc — **Tedai Kuroiwa**, the gracious door

- The magistrate's agent (*tedai*) who *records* your achievements and secretly architects the rice-quota skim.
  Outwardly an ally; **defeated by evidence, not violence.** Sits **beneath** Daikan **Iemasa** and
...
- **Resolution (multi-route, partial justice):** **Peaceful** — outgrow him; make his skim irrelevant; you need
  ...
  not the MC. **[MOTIF: rigged box]** the through-line connects here (Magobei's *masu* → Kuroiwa's
  weighing-yard).

## T4.4 Estate roster & buildings — E3 → E4

**Joins:** a patrol-leader NPC + the MC's 2–3-man detail; a counting-house clerk under Tanomo;
...
**Buildings:** counting-house, guest quarters, roofed dojo + barracks → a proper **wall + gatehouse**,
...
emotional callback is *carried forward to T5* (no second mechanical payoff).

## T4.6 Areas & mobs

**Areas:** **The Daikan's Office (代官所)** *(spine-critical for formal T4 recognition; the racket's
...
*Marutaya* debt-restructuring → *goyōkin*). **Mobs:** Tomita toughs; the corrupt official's muscle /
...
the through-line; the incurious one advances by economy/society alone. **No T4 castle-town folklore "wink"** —
there is no invented ghost-story beat here (it would breach the ≤1 ambiguity cap; §1.13).

> **Scope of the castle-town chapter.** Its fuller depth — estate stages **E4–E5** in full; the marriage/adoption
> brokerage depth; the *daikan*-office audit/evidence sub-thread depth; the full social-district introductions
> system — belongs to this chapter (E3 Prosperous itself lands earlier, at the Region).
> **Out of scope:** the Matagi hunters, the Pilgrimage Order, and the Scholars-&-Physicians
> as a *network* (keep Sōan / Obaa Kuni as the existing seed only).

---

# T5 — Edo (the endgame horizon)

> **The national tier is the game's designed endgame horizon** — the arc the castle-town cliff-hanger points
> toward. All beats below are the authored target for the capital chapter, kept lean.
...
## T5.2 Main-quest beats

1. **The two conduits open (seeded late T4).** Genemon hands the MC a share of outfitting the **daimyō's**
...
## T5.3 Antagonist arc — the Edo factor + the impartial test
...
## T5.4 Estate roster & buildings — E5 Restored-and-Surpassed

**Joins/escalates:** **Rusui Mukai Settsu-no-suke** (the MC's sole pen-pal proxy in the capital — the
single recurring T5 through-line face; the filter through whom every official contact passes); **Procession-
...
**Buildings:** a rebuilt grand *omoya* beyond the three-generations-ago original; a famous-*meibutsu*
...
## T5.5 Endgame, succession & epilogue
...
**CONDITIONAL on completing the optional, missable Origin O5 track** — the recovered family (incl.
...
- **Numbers belong in §4.** Every "result," "threshold," "result-recorded," and pillar-gate here is a **shape**;
  ...
  only the *story shape.*
- **Rung copy belongs in §3.** T3's region-ladder rung wording, all per-tier reveal cadences, **the staggered
  combat-reveal ladder** (one reveal per beat — R3 starter weapon + auto-resolve + retreat + Bestiary → R4
  durability bands + Crafting → R5 stance → weapon-L10 ability/item slots → **T1 2nd line (sword)** → **T2 3rd
  line (staff, pulled forward)**),
  and the **per-rung curated-activity sets** that feed the rung-meter are **§3's** job; §5 fixes only the
  *story shape* of each ladder and the **Phase-1 / Phase-2 narrative arc.**
- **Authenticity anchors.** The **Standing & Office** pillar kanji is **官威 (*kan'i*)**, "authority of office";
  the R7 rung title is **地方役 (*jikata-yaku*)**, "home-field bailiff"; *michi-ban* (road-warden) is
  **diegetic-only** — an in-house role the estate confers, not an attested bakufu office; and the **T5** arc has
  the house **staff and run the *domain's* Edo establishment** (the *rusui-yaku* under the daimyō's
  *sankin-kōtai*), never its own.
...
- **Period names are invented stand-ins.** §5's cast uses fictional period names — **Heita**, **Obaa Kuni**,
  **Mosuke**, **Hayami**, **Mukai**, **Shigemasa**, **Kihei**, **Sōan** and the rest — coined off real-figure
  echoes so no attested Edo name surfaces as canon; **Nihonbashi** is the one allow-listed real place name.
  §1.8 is the authoritative name registry; §6.6 holds the real-name denylist.
