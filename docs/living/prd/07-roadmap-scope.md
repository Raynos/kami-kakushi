...
| **Crafting** | **hybrid**: simple recipes from T0-R4; the component/quality system from T2-V3 (the Village). **Crafting stays a homed section** (of Character), NOT its own tab; the **quest log regains its OWN top-level tab**, making a **seven-tab IA** (Work · Map · Estate · Inventory · Character · Combat · **Quests** — **D-119**, reinstating D-037, superseding D-112's six-tab lock; exact tab per `ui-design.md`) | §4.7.2, §2.11/§2.12, §3.4 |
