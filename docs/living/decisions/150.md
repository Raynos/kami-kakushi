...
- **Consequences.** Build plan:
  [`fable-2026-07-13-merchant-state.md`](../../../project/archive/fable-2026-07-13-merchant-state.md)
  (~~PROPOSED~~ → **LANDED 2026-07-14, session-204**; human calls:
  partial-drift restock · the offer sags to REFUSAL at 0 · explicit
  purse readout. Magnitudes sim-verified under ADR-132 — the landing
  pass rescaled `ESTATE_DEED_PER_ACT` 0.6 → 0.8, the D-133 ratio's
  documented lever, because the killed exploit had been banking
  zero-clock treasury deeds). Extends **ADR-163 §5**; composes with
  ADR-099 (personal coin lane). Save shape: merchant state is new
  persisted state — the v13→v14 schema bump landed with the plan.
...
### ADR-199 ✅ — the shared tree's index is SOLVED; hunk-level staging is not the protocol; worktrees are opt-in (🔁 operationalizes ADR-196)

- **created_date:** 2026-07-14 (session 206)
- **Context.** The question was raised directly: should agents get
  **separate staging areas** (`GIT_INDEX_FILE` + `git add -p`) so
  each can commit only its own hunks in the shared tree — or is
  `git commit -- <file>` enough? Investigated against a throwaway
  repo rather than from memory; ADR-196 had already ruled the
  **hybrid** architecture but never wrote the operational half.
- **Decision 1 — `git commit -- <path>` is SUFFICIENT for the index.**
  The `--only` form already *is* a private index: git builds a temp
  index from HEAD + the named paths and never consults the shared
  one. `GIT_INDEX_FILE` + `read-tree HEAD` is the same trick by hand.
  The only delta is hunk granularity **within one file**.
- **Decision 2 — hunk-level staging is RULED OUT as a routine
  protocol; it stays a RESCUE TOOL.** Verified: it works, but the
  tree it commits (HEAD + your hunks) is one **no gate ever ran
  against** — `verify` runs on the *working* tree (HEAD + your edits
  + every co-agent's in-flight edits). So it trades a file-level
  false green for a finer-grained one, and "which hunks are mine" is
  partly guesswork about whether a co-agent's line is a finished
  thought. It earns its place only to *extract* work from a tangle.
- **Decision 3 — worktrees are OPT-IN, by job size (human ruling).**
  "Larger plans / agents can choose their own worktree; smaller
  agents go fast on `main`." This is ADR-196's hybrid, now with
  criteria: opt in for a multi-file sweep / hot-file work / a long
  red window / hours-long or broadly-regenerating jobs; stay on
  `main` for single-surface work that can be green in minutes.
- **Why it matters.** The index was never the open problem — the
  **shared working tree** is: (a) a pathspec commit takes the WHOLE
  working-tree copy of a file, so a co-agent's uncommitted edits in
  that same file ride along; (b) you verify a tree you do not commit.
  Both are working-tree problems that *only* isolation closes, and
  finer staging makes (b) worse, not better. The render-split
  (ADR-196) is what makes (a) rare by construction.
- **Consequences.** New guide
  [`docs/guides/shared-tree-git.md`](../../guides/shared-tree-git.md) —
  the model, the pathspec contract, the `git commit -p` trap (it
  seeds from the SHARED index and sweeps; guard-blocked, correctly),
  the rescue recipe with its three silent traps, and the worktree
  opt-in criteria. Two worktree costs documented from disk: it needs
  its **own `node_modules`**, and it is **not what `:5173` serves**
  (the shared dev server runs from the main tree root — so worktree
  work is unreachable in the shared playtest, which collides with
  PH6; playtest after landing, or run a private vite on another port
  via `KAMI_ALLOW_MULTI_DEV=1`, never killing the shared server).
  `working-agreements.md` Checkpoint §1 points here.
- **Refs:** ADR-196 (hybrid tree, locks, hot-file splits — this is
  its operational half) · the cede/fold norm · PH3 · PH6.

### ADR-200 ✅ — talking is ASKS: state-derived answers own everyday talk; the C4.2 press-A cursor is retired

- **Date:** 2026-07-18 (session-210) · **Status:** ✅ built (same day)
- **Context.** FB-415: *"Characters are a button and you get a random
  low context line… flavor text goes into the Story panel."* The
  2026-07-17 grill locked eight decisions (D1–D8, the brainstorm +
  the plan `fable-2026-07-17-talk-system-redesign.md`); a pre-build
  Q&A (2026-07-18) added four rulings — all-Fable build, step 4 on
  the agent's self-pick, defaults accepted, and the **full-T0
  floor**: every person×rung talk that existed keeps working.
- **Decision.** Everyday talk is an **ask**: a rung-windowed,
  when-gated question whose answer derives from LIVE GameState,
  rendered inline in the talk surface and **never written to any log
  channel** (D4 — authored story beats alone write to Story). One
  selector (`availableAsks`) feeds surface and reducer (AC-6); asks
  are free (D7); heard-state is a **freshness handshake** (`asksHeard`
  stores the answer's key at hear time; default key = a digest of the
  answer's own text, so a state move that reshapes the answer
  re-lights the ask — D6, no timers). Asks are authored in
  `narrative/asks.md` (`## ask` grammar → `asks.gen.ts`); real logic
  rides the closed `ask-natives.ts` / `ask-refresh.ts` vocabularies.
- **The re-homing (D8).** Every vn person's u9-* cast def answers
  through ONE person-ask (`native: u9-<npc>` — gate/memory-aware,
  takes-overlay-aware, every satisfied line at once); nothing cut;
  u9-yohei stays with the stall (D2/HR-44). The `talk_to` intent,
  its reducer case, and the "Ask X more" flip are **deleted**;
  `deliverDialogue` remains for the cold open + rake teach.
  `asksHeard` is SCHEMA_VERSION 15 (additive; hydrates `{}`).
- **Surface (ADR-075).** Diverge `talk` — A in-row ask plates
  (ships) · B VN-lite exchange · C mini-transcript (DEV-only), all
  on the same selectors; the 新 unheard-count mark + dim-but-
  pressable heard plates are shared mechanics. Pick at **HR-45**.
- **Refs:** FB-415 · D1–D8 (2026-07-17 brainstorm) · ADR-075 ·
  ADR-139 (answer waves pending) · AC-6 · TST1/TST4 · PH6.

### ADR-201 ✅ — the stamp book is a COMPACT seal-strip: the MC's own book, afterglow + Character-tab home (human rulings, live grill)

- **created_date:** 2026-07-18
- **Context:** the E3 stamp-book resume plan
  (`docs/plans/fable-2026-07-18-stamp-book-resume.md`) carried an
  open home HD-item and an unresolved document fiction; the human
  chose to rule them LIVE (session 214 grill,
  `project/brainstorms/2026-07-18-stamp-book-decisions.md`) rather
  than via async HD/HR filings. Mid-grill she also re-scoped the
  surface's SCALE — the full-screen `openStampBook()` prototype is
  "huge… not like a full screen or huge modal thing"; what she wants
  is *"a smaller UI element… maybe 20% of the screen or less… see
  your progress, and see the rungs you're going to collect in the
  future, like pokemon gym badges."*
- **Decision.** Five rulings, all human-locked:
  1. **Home = ceremony afterglow + a revisitable home** — the book
     presents itself right after each rank ceremony (the fresh seal
     still wet, TST3), AND stays revisitable. The plan's step-3 HD
     is CLOSED by this ruling; no HD-item is filed.
  2. **The revisitable home is a Record panel on the Character
     tab** — the backward-looking "who am I / what have I done"
     surface; Progress stays the forward tracker (TST1).
  3. **Fiction = the MC's OWN seal-book** (shuinchō register the
     player carries); each rank-granter presses their own seal.
     Every named hand grounds in bible canon (02-house / 03-tiers);
     a NEW bible fact goes through the HR proposal, never freehand.
  4. **Scale = compact, ≤20% of the screen** — an in-page section,
     gym-badge style: pressed seals + empty FUTURE slots, never a
     full-screen modal. Detail is a **per-seal inspect popover**
     (whose hand, day/season, crisis knots); no large element
     anywhere. The full-screen prototype retires to DEV reference.
  5. **Future slots: next named, rest mystery** — the immediate
     next rung reads; slots beyond it are blank silhouettes.
- **What this does NOT change:** ADR-075 still applies — the compact
  section ships from 2–3 working variants (the plan's A/B/C
  directions reinterpreted at compact scale) behind the DEV Review
  toggle, picked at its HR-item.
- **Refs:** graphics concepts #8 ❤️ + #10 · the resume plan ·
  the 2026-07-18 grill capture · ADR-075 · ADR-139 (caption text) ·
  TST1/TST3 · PH4 (live rulings beat parked filings).
