...
### D-136 ✅ — process: F8 real-play telemetry — attended time is the third pacing leg; agents consume it by default
- **created_date:** 2026-07-05
- **Context:** pacing stood on two legs — sim bots (theory, D-132/F4) and
...
- **Decision — the delivery loop (Ph4 redirected off the F3 inbox):**
  reports are NOT playtest captures. They land in **git-ignored
  `project/telemetry/`** (committed README only): **folder** (auto-drop on
  session-end via `apply:'serve'` middleware) + **signpost** (D-132 balance
