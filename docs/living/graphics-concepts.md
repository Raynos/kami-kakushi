...
| 12 | **VN scene-card pilot** | The COLD OPEN (R0) | ONE composed woodblock vignette from existing primitive vocabulary; "really really really hard" — kill fast if it slops. **Demo built (s122, human-pulled, one version, two cards):** `src/ui/scene-cards/`, DEV → Story → "⤢ Scene cards" — Sōan's sickroom + Genemon's grain-store; awaiting the human's look; the real E2 track (grammar spec → HR → one-moment diverge → keep/kill) unchanged. |
