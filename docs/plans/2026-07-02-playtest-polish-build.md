...
1. **DEV panel restructure (F16)** — a sub-header of two sections **Settings** ·
   **Variants**; the Variants list is a **collapsed per-surface summary**, click
   ...
   back._
2. **Cold-open slow reveal + padding (F14)** — the frame gets roomier padding
...
3. **Story cascade AFTER open-eyes (F15)** — the post-awake narration currently
...
4. **Filterable log (F9)** — **DIVERGE:** A bottom tabs (default) · B toggle
