...
**Status:** 📐 PROPOSED — designed in the D-139 grill session
(2026-07-06); build is a follow-on workstream, not started.
...
**ADR:** D-139 · **Skill:** `.claude/skills/narrative-diverge/SKILL.md` ·
**Brainstorm:** `project/brainstorms/2026-07-06-narrative-diverge-design.md`

## Why

D-139 makes every narrative element ship from 3+ takes, and the human
...
reviewed from the R-item doc alone (the skill's §2.7 interim note).
...
Routing is the human's call at kickoff (D-124).

## Shape (locked in the grill; details are the builder's)

1. **DEV-only narrative-variants source area.** Alternate takes live
   compiled-but-strip-gated (D-138's `__DEV_TOOLS__` axis is the current
   inclusion gate; post-T0 they fold to dead code like the rest). Canon F5
...
- These are new UI surfaces → D-075 applies to *them*; scope the diverge
  touch for DEV-only tooling in the build (a lighter pass is arguable —
  decide and record at kickoff, don't skip silently).
- Both surfaces are `__DEV_TOOLS__`-gated (D-138 ship/strip modes must stay
...
- First real consumer: whichever narrative work runs first under D-139
...
- Prod bundle (strip mode) carries zero take-set strings (D-138 gate green).
