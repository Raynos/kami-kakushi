...
- **Idle TTL is TWO-TIER** (deviates from §7.1's flat default): 5 min with
  autos armed, **2 min static** (no autos) — a static screen with no input
...
- Note re-engage: **keep**, 30 s window (§7.2 default).
- Blur: **exclude always** (§2.2c as written).
- Micro-gaps: **no merging in v1** (§7.3 default).
- History: **keep across new-game**, run-id tagged, clear button (§7.4).
...
consumer is NOT the human reading a DEV-panel table — it's **the next agent
...
feature's value is only realized by Ph4's delivery loop — the data must sit
on paths agents already walk (the session brief, the D-132 balance flow,
the committed distillation trail). Ph1–2 are the trust layer (a lying
stopwatch confidently automates balance-slop — worse than no data); Ph3 is
...
### Ph4 — ~~inbox adapter (BLOCKED on the capture-inbox plan shipping)~~ local telemetry drop (human-redirected 2026-07-05)
...
**`project/telemetry/`** — **git-ignored** (a committed `README.md` +
...
dev-middleware (`apply: 'serve'` — structurally absent from builds) that
writes `<runId>-<stamp>.md` into `project/telemetry/`.

**The delivery loop (human-locked 2026-07-05) — four pieces, because a
folder alone is worthless (agents are session-goldfish; data must sit on
paths they already walk):**

1. **The folder** — `project/telemetry/` holds the dropped reports;
   git-ignored except `README.md`. Local sensor data, regenerated by
   playing; never repo history.
2. **The signpost** — the D-132 balance flow in `qa-playtesting.md` §2
...
3. **The morning shout** — `session-brief.sh` prints a one-liner when
...
4. **The diary rule** — stated in the folder README: raw reports are
   ...
   numbers quoted — the `brainstorms/raw/` two-tier pattern. The committed
...
exists and stays silent when none do; the `qa-playtesting.md` §2 step +
