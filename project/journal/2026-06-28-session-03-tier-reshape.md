...
- `project/status/pending-prd-changes.md` — **NEW**: the "ADRs locked but not applied to PRD" ripple tracker
  (per-file checklist for prd.md / living docs / code).

## Next intended steps
1. **Ripple the reshape into the PRD + living docs + code** — the `pending-prd-changes.md` checklist; ideal as
...
- **The PRD is STALE on the tier model** until the ripple lands — `prd.md` still says 5 tiers (T0–T4). Read it
  with `pending-prd-changes.md` open; the ADR batch + the feedback capture GOVERN (D-022). Don't trust the PRD
