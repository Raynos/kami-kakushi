...
## Resolution (answers, 2026-06-26)
- **FU1 ✅** Build the FULL multi-backend save layer in M0 (backend-abstraction + atomic write + magic field + newest-wins selector). Defer only the rich per-system fields (additive later).
- **FU2 ✅** Newest-save selection: timestamp sourced in the SAVE LAYER (a documented core-lint exemption — metadata, not game logic) + a monotonic save-counter as the real selector, timestamp as tiebreaker. The deterministic core stays clock-free.
- **FU3 ✅** RNG = `{ seed, cursors:{combat,loot,seasonal,worldgen} }` (persisted, monotonic per stream); weather/lunar = a PURE stateless `deriveDayKeyed(seed,'weather',day)` helper, NOT a persisted stream.
- **⚠ FU4 — NO generic reveal-queue.** **Design the unlock schedule so reveals are inherently ONE-AT-A-TIME** (staggered by construction; no two collide on a beat). Genuine multi-element single-feature reveals are **bespoke one-offs designed with the human per case**. Reveal cadence is a gameplay/UI DESIGN responsibility, not a runtime queue. *(Dissolves the simultaneous-reveal determinism concern.)*
- **FU5 ✅** M0 GameState = `{hp, satiety, attributePoints}` + `character.level` (=1 floor, Q1) + satietyMax-at-floor. Do NOT pre-declare subEngines/CombatEncounterState/dialogue — add additively at their milestone (Q45).
- **⚠ FU7 — SEQUENTIAL per-tier progression (reshapes §4.8.1's parallel accrual).** **Phase 1: climb the rungs** (R0→R7) via a **curated per-rung activity set** (1-to-many designed activities per rung, NOT a single repeat-counter), tracked by the rung-meter + story (Q30/Q6). **Phase 2: the estate-influence grind** unlocks AFTER the final rung — "really level up the estate", grind the four pillars to the Q7 tier-gate thresholds → tier-up. **Pillar DEEDS are gated to the post-rung phase** (not accruing across rungs) → prevents "half the rungs but maxed deeds." Per-rung activities authored separately from the pillar-deed inventory. *(Also resolves FU11: rung-meter+story gates each rung; the Q7 hybrid pillar distribution is the separate phase-2 tier-gate.)*
- **⚠ FU8 — PER-SKILL perks, stackable, NO hard global cap (risk accepted).** Every skill (combat + labour) has a perk / flat-bonus track unlocked by leveling it (~2-8 perks OR ~3-8 small flat stat bonuses each). They stack — but gear/level/attributes stack too and enemy difficulty + drops scale alongside, so it self-balances **holistically**. The real bound is the **incremental skill unlock** (skills reveal per rung/tier — can't front-load combat-via-labour). Verifier checks each perk's magnitude is small (not ==0, not a global ≤CAP). Conditioning stays the zero-stat enablement gate.
- **FU9 ✅** Fun-proxies (dead-time, reward/unlock cadence, visible-next-goal, first-5-min hook) instrumented at M1 as **REPORT-ONLY** (gate at M6); set + thresholds from fun-factor.md (deed-cadence T0 ~5 min).
- **FU21 ✅** Labour satiety throttle: **flat above ~0.7 satietyMax, knee to a ~0.5 floor**; modest drain per action; rest fully refills; satietyMax = base + small per-level growth.
- **⚠ FU10 — Hybrid-gate thresholds need a DEDICATED V2 rebalance** (NOT simple ratios — good=0.2/great=0.5 "breaks"). Author **per pillar per tier**, balanced against the **fixed deed inventory**. Semantics: **good = the expected baseline ("a good job") · great = really strong / well done · excellent = above-and-beyond.** A §4 balance overhaul; tuned in playtest.
- **FU12 ✅** Combat-reveal ladder: R3 = single starter weapon + bare auto-resolve loop + retreat → durability bands at R4 (loot→craft) → stance slot at R5 → ability/item slots at the first weapon-line L10 milestone → 2nd line at T1, 3rd at T2. One reveal per beat.
- **⚠ FU13 — MORE weapons (content expansion).** A growing roster, still incremental: **T0 unlocks 2 new weapons · T1 +3 · T2 +4** (~9–10 across v1; T0 still starts with the one starter). Author a period-appropriate set (yari/uchigatana/kodachi/bō/kusarigama/naginata/tanbō/kanabō/yumi…), each with archetype params (baseSpeed/reach/targetCount) + a signature ability, in V2.
- **FU14 — three INDEPENDENT combat tracks, cleanly separated** (rewrite the conflated §2.8): **character level & XP = combat & kills** (drives HP + attribute points, Q1); **Arms pillar = DEEDS** (the influence economy); **Combat-Standing rung-meter = per-rung curated activities** (whatever fits each rung/area — NOT raw XP/kills). One kill feeds level; deeds feed Arms; rung activities feed the meter.
- **FU15 ✅** mobLevel = an explicit per-mob `MobDef.level` field (hand-tunable; defaults ~ the dangerRing's expected character-level).
- **FU16 ✅** Satiety→combat = a `satietyRate` multiplier on attackPower (lighter on attackSpeed), flat above ~0.7 satietyMax → ~0.5 floor; 'adequate satiety' = ≥~0.7, where the locked 20-35% first-fight win-rate is measured. Separate combat coefficient from labour.
- **FU17 ✅** Durability = 4 integer bands on weapon attackPower (75+/50+/1+/0 → 1.0/0.9/0.75/0.55); FIXED wear per FIGHT (cheap, replay-stable); armour bands identically on defense; repair/recraft restores; never auto-unequip.
- **⚠ FU18 — The ~28.5h budget is a FLOOR, not a ceiling** (reverses D-016's "locked budget as target"). The game can/should be LONGER — a long **OSRS-rough grind**, enough grinding content that it isn't quick to finish; interleave content, **don't brick-wall**; a "leave it auto-running for a few hours, check your progress" feel. § 4.8 pacing becomes a *minimum-grind* model, not a fixed total. *(→ annotate D-016; raises FU23 the idle question.)*
- **FU19 ✅** [tune-later] Win-rate bands: each key combat rung humbling-but-winnable fresh (~30-45%), comfortable after that rung's training/gear (R3 20-35%→~85% set); headless-tested, tuned in playtest.
- **⚠ FU20 — BROADER cross-pillar combos** (multiple pillar pairs / larger magnitude — more T2 variety, a wider §4.3 exception). The **trade-≤⅓ cap stays HARD** — combos computed AFTER the trade clamp; the §6.6 verifier proves trade can never breach ⅓ via a combo.
- **FU22 ✅** Intra-line dialogue = a flat choice list per node (`choices[]` + `ChoiceId`); a choice's effect = `locksLineIds[]`/flags. Data, not scripting; deterministic; only chosen-flags persist (save-light). `content/dialogue.ts`.
- **FU23 ✅** v1 stays **active-only** (no offline catch-up): with the tab OPEN, idle auto-resolve combat + auto-repeating labour run unattended for hours — the "leave it running, check progress" feel comes from auto-resolve/auto-repeat, NOT an offline layer. Auto-producers stay T3+. *(Reconciles FU18's long-grind with the active-only lock.)*

**WAVE-2 COMPLETE ✅ — all 23 follow-ups answered.** Key direction shifts feeding V2: FU4 design-staggered reveals (no queue) · FU7 sequential per-tier (rungs→estate-grind) · FU8 stackable per-skill perks · FU10 gate-threshold overhaul · FU13 more weapons · FU14 three clean combat tracks · FU18 budget = FLOOR (longer OSRS-grind) · FU20 broader combos · FU23 tab-open auto-resolve. **Next: the PRD-V2 plan → reshape prd.md → V2 → build M0/M1.**