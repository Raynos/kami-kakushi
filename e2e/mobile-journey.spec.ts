// Mobile play journeys — REAL taps (touch events, not synthetic dispatch) through
// the player-facing flows. `__qa` is used only to observe state, never to act:
// if a tap stops landing (an overlay paints over a verb, a tab drifts offscreen,
// a modal traps the page), these go RED. qa-playtesting.md §1 "Mobile e2e lane".
import { expect, test } from '@playwright/test';
import { boot, expectNoHorizontalOverflow, expectNoPageErrors } from './helpers';

test('cold open: tapping "Open your eyes" wakes the game', async ({ page }) => {
  const errors = await boot(page);
  await page.locator('button.verb.primary').tap();
...
test('R3: tabs switch by tap, actions land, the wolf fight resolves', async ({ page }) => {
  ...
  await page.locator('button.verb', { hasText: 'Rest a moment' }).first().tap();
  await expect
  ...
  await page.locator('button.verb', { hasText: 'Face the wolf' }).tap();
  await expect
