...
describe('findOrphanedIds — the save-vs-src/ id diff (step 4)', () => {
  it('a fresh save is clean (the sensor does not cry wolf)', () => {
    ...
    expect(formatOrphanReport(report)).toBe('no orphaned ids — this save matches src/');
  });

  it('sees a reveal id that src/ renamed away', () => {
    const report = findOrphanedIds(base({ seenReveals: ['room-kura-but-renamed'] }));
  ...
  it('groups several orphans across kinds and totals them', () => {
    ...
    expect(report.groups.map((g) => g.kind).sort()).toEqual(['belongings', 'seenReveals']);
  ...
  it('flags are NOT diffed (free-form facts have no roster to diff against)', () => {
    const report = findOrphanedIds(base({ flags: { 'a-flag-no-registry-knows': true } }));
    expect(report.total).toBe(0);
  });
});
