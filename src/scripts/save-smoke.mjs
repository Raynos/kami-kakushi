...
try {
  ...
  await page.goto(`${BASE}/?fixture=wealthy-idler`, { waitUntil: 'networkidle' });
  ...
  if (before.keyed === 0) fail('fixture produced no keyed log entries — smoke proves nothing');
  ...
  const after = await page.evaluate(() => {
    const es = window.__qa.state().log.entries;
    return { count: es.length, sig: es.map((e) => [e.key, e.contentKey ?? '', e.text]) };
  });
  ...
  if (JSON.stringify(after.sig) === JSON.stringify(before.sig)) {
    ok(`log rehydrated byte-identically across reload (${after.count} entries)`);
  } else {
    fail(`log differs after reload (before ${before.count}, after ${after.count} entries)`);
  ...
  if (painted > 200) ok(`UI painted the restored state (${painted} chars of text)`);
