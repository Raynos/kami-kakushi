...
function generateTokens(): string {
  ...
  if (start < 0 || end < 0)
    throw new Error(
      'gen:docs: no :root token block found in styles/tokens-base.css',
    );
  return [
...
function generate(): string {
  ...
  for (const a of ACTIVITIES) {
    const yields = Object.entries(a.yields)
      ...
      .join(', ');
    L.push(
      `| ${a.id} | ${a.skill} | ${a.area} | ${yields} | ${a.satietyCost} | ${a.xp} |`,
    );
  }
  L.push('');
  ...
  const timingRow = (name: string, t: ActionTiming): string =>
    t.kind === 'timed'
      ...
      : `| ${name} | instant | — | — |`;
  for (const a of ACTIVITIES)
    L.push(timingRow(`do_activity:${a.id}`, ACTIVITY_TIMING[a.id]));
  ...
  L.push('');
  L.push(
    `> Node ceiling: \`MAP_NODE_CEILING = ${MAP_NODE_CEILING}\` — the T0 hard cap on`,
  );
  L.push(
  ...
  for (const m of MOBS) {
    L.push(
      `| ${m.id} | ${m.label} | ${m.level} | ${m.scripted ? 'yes' : ''} |`,
    );
  }
  L.push('');
  ...
  for (const [foe, d] of Object.entries(MATERIAL_DROPS)) {
    L.push(
      `| ${foe} | ${d.material} | ${d.qty} | ${d.chanceNum}/${d.chanceDen} |`,
    );
  }
  L.push('');
  ...
  for (const e of ESTATE_STAGES) {
    L.push(
      `| E${e.stage} | ${e.label} | ${e.coinCost} | +${e.satietyMaxBonus} |`,
    );
  }
  L.push('');
  ...
  for (const it of MARKET_ITEMS) {
    const grants = Object.entries(it.grants)
      ...
      .join(', ');
    L.push(
      `| ${it.id} | ${it.label} | ${it.coinCost} | ${grants} | ${it.stockCap} |`,
    );
  }
  L.push('');
  ...
  for (const t of HOME_TIERS)
    L.push(`| ${t.tier} | ${t.rung} | ${t.label} | ${t.kanji} |`);
  L.push('');
  ...
  for (const b of BELONGINGS) {
    const comfort = b.homesCook
      ...
      : b.comfort
        ...
        : 'keepsake';
    const source =
      b.source.kind === 'buy' ? `buy ${b.source.coinCost} coin` : 'granted';
    L.push(
      `| \`${b.id}\` | ${b.label} | ${b.kanji} | ${comfort} | ${source} |`,
    );
  }
  L.push('');
  L.push(
...
if (check) {
  for (const [path, content] of outputs) {
    ...
    if (existing !== content) {
      console.error(
        `gen:docs --check FAILED: ${path} is stale. Run \`pnpm run gen:docs\`.`,
      );
      process.exit(1);
    }
  }
