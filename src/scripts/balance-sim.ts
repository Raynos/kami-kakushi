...
function reportFingerprint(): string | null {
  ...
  return line
    ? (line.slice(FINGERPRINT_PREFIX.length).split('`')[0] ?? null)
    : null;
}
...
function generateReport(matrix: PersonaRuns[]): string {
  ...
  L.push(
    `${FINGERPRINT_PREFIX}${inputFingerprint()}\` (the evaluated design inputs —`,
  );
  L.push(
  ...
  L.push(
    `  still convert at the ${balance.AUTO_REPEAT_MS} ms heartbeat and are labelled as such.`,
  );
  L.push(
...
function selftest(): number {
  ...
  for (const rung of ['R0', 'R1', 'R2'] as const) {
    ...
    if (
      ...
      ok(
        `${rung} equals walkPacing (${w.acts} acts, ${w.rests} rests, ${w.meta} meta)`,
      );
  }
  ...
  if (JSON.stringify(sim) !== JSON.stringify(again))
    ...
    ok(`deterministic on seed ${CANONICAL_SEED} (${sim.totalIntents} intents)`);
  ...
  for (const seed of SIM_SEEDS) {
    ...
    if (!m.ascended || m.softLock) {
      fail(
        `seed ${seed}: did not ascend (softLock=${JSON.stringify(m.softLock)})`,
      );
    } else
      ok(
        `seed ${seed} ascends (${m.totalIntents} intents, ${min1(m.totalWallMin)} min)`,
      );
  }
  return failures;
}
...
function check(): number {
  ...
  const p2 = greedyRuns.map(
    (r) => r.rungs.find((x) => x.rung === PHASE2_RUNG)?.wallMin ?? 0,
  );
  const p2min = `[${min1(Math.min(...p2))}–${min1(Math.max(...p2))}] min`;
  if (rv.built === 0) {
    console.log(
      `  · Phase-2 ratio: no built Phase-2 economy in the matrix — gate no-op.`,
    );
  } else if (rv.ok) {
    console.log(
      ...
      if (s.ok) {
        const closed =
          s.promise === 'ladder'
            ...
            : 'ladder + ascension';
        console.log(
          `  ✓ ${s.personaId} seed ${s.seed}: ${closed}, no soft-lock`,
        );
      } else {
        reds++;
        console.error(
  ...
  if (committed === live) console.log(`  ✓ report fresh (fingerprint ${live})`);
    ...
    console.error(
      `  ✗ RED report stale: ${OUT} header says ${committed ?? '(missing)'}, live inputs ` +
        `hash ${live} — run \`pnpm run balance:report\` and commit the diff.`,
    );
  }
...
function parseReportMedians(text: string): Map<string, number> {
  ...
  const start = lines.findIndex((l) =>
    l.startsWith('## greedy — time-to-rung'),
  );
  if (start < 0) return out;
  for (
    let i = start;
    i < lines.length && !lines[i + 1]?.startsWith('## greedy — arc');
    i++
  ) {
    const m = /^\| (R\d) \| \d+ \|(?:[^|]*\|){4} ([\d.]+) \|/.exec(
      lines[i] ?? '',
    );
...
function summary(): void {
  ...
  console.log(
    'balance-sim --summary (greedy medians vs the HEAD-committed report):',
  );
  console.log('```');
  console.log('rung   wall-min   Δ vs HEAD   band verdict');
  for (const v of greedyBandVerdicts(runs)) {
    const med = median(
      runs.map((r) => r.rungs.find((x) => x.rung === v.rung)?.wallMin ?? 0),
    );
    const prev = before.get(v.rung);
    const delta =
      prev === undefined
        ? '—'
        : `${med >= prev ? '+' : ''}${(med - prev).toFixed(1)}`;
    console.log(
  ...
  if (args.includes('--check-fresh')) {
    ...
    console.error(
      `balance report STALE: ${OUT} header says ${committed ?? '(missing)'}, live design ` +
        `inputs hash ${live}. Run \`pnpm run balance:report\`, eyeball the diff (it IS the ` +
