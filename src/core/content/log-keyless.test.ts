...
const KEYLESS_ALLOWED: readonly {
...
const allowed = (text: string): boolean =>
  KEYLESS_ALLOWED.some((a) => a.match.test(text));
  ...
  it('a full arc emits ZERO keyless log entries', () => {
    ...
    if (offenders.size > 0) {
      const worst = [...offenders.entries()]
        ...
        .slice(0, 15)
        .map(
          ([text, m]) => `  [${m.channel}] ×${m.count}  ${text.slice(0, 90)}`,
        )
        .join('\n');
      throw new Error(
  ...
  it('no durable-channel log literal in src/core lacks a contentKey', () => {
    ...
    for (const file of coreSources(root)) {
      ...
      for (const m of src.matchAll(
        /channel:\s*'(?:narration|system|milestone)'/g,
      )) {
        ...
        const before = src.slice(
          Math.max(0, src.lastIndexOf('{', m.index!) - 240),
          m.index!,
        );
