...
describe('renderer variant routing — Map travel presence (D-075, FB-340)', () => {
  let root: HTMLElement;
  beforeEach(() => {
    document.body.innerHTML = '';
    window.matchMedia = (q: string): MediaQueryList =>
  ...
  it('registers travel-presence with the glide default (A ships)', () => {
    const dev = createDevApi();
  ...
  it('the here-ring carries its presence hook on the prod path (dev undefined)', () => {
    ...
    render(mapState(), null);
    openMap();
    expect(root.querySelector('.map-pane .sheetmap-here-ring')).not.toBeNull();
  });
});
...
// FB-157/M6 — the diverged ESTATE SECTION (ADR-075): B/C route through the DEV harness and
// replace the default de-framed cards; the Improve button drives the real improve_estate
// intent in every take. RED-proof: kill the router line and B/C fall back to the default.
describe('renderer variant routing — Estate section (FB-157/M6)', () => {
  let root: HTMLElement;
  beforeEach(() => {
    document.body.innerHTML = '';
    window.matchMedia = (q: string): MediaQueryList =>
  ...
  function estateState(): GameState {
    const base = createInitialState(1);
    return {
      ...base,
      flags: { ...base.flags, awake: true },
      resources: { ...base.resources, coin: 100000 },
      unlocked: [...base.unlocked, 'readout-rice', 'panel-estate'],
    };
  }
  function openEstateTab(): void {
    [...root.querySelectorAll<HTMLButtonElement>('.nav-tab')]
      .find((b) => (b.textContent ?? '').includes('家'))
      ?.click();
  }

  it('default A renders the de-framed rung-card sections', () => {
    const dev = createDevApi();
    const render = mount(root, () => {}, noopHooks(), dev);
    render(estateState(), null);
    openEstateTab();
    expect(root.querySelector('.estate-pane .rung-card')).not.toBeNull();
  });

  for (const vid of ['estate-b', 'estate-c'] as const) {
    it(`variant ${vid} routes (no default rung-card) and its Improve drives improve_estate`, () => {
      const seen: Intent[] = [];
      const dev = createDevApi();
      dev.setVariant('estate-section', vid);
      const render = mount(root, (i) => seen.push(i), noopHooks(), dev);
      render(estateState(), null);
      openEstateTab();
      const pane = root.querySelector<HTMLElement>('.estate-pane')!;
      expect(pane.querySelector('.rung-card')).toBeNull(); // the default was swapped out
      const improve = pane.querySelector<HTMLButtonElement>('button.verb');
      expect(improve).not.toBeNull();
      improve!.click();
      expect(seen).toContainEqual({ type: 'improve_estate' });
    });
  }
});
